...
<el-container class="sub-container__table" v-if="!permission.table.${layout.options.boSubEntity}.hidden">
  <el-header class="sub-header__table" sunBos="${sunBos}" style="height:${subHeadHeight}${subHeadHeightUnit};line-height:${subHeadLineHeight}${subHeadLineHeightUnit};background: ${subBgColor};color:${subFontColor}; font-weight: ${subFontWeight};font-size: ${subFontSize}${subFontUnit};margin:0 20px;">
  	<div class="flex" style="justify-content: space-between;">
  		<#--  ccwgq 2022-04-08 增加头部计数的功能  -->
     <#if subCounter=true><span>${layout.desc}({{data.${subTablePath}.length}})</span>
              ...
              <tbody>
                  <tr v-for="(item, index) in <#if layout.subtablePagination>pagingSubData('data.${subListPath}')<#else>data.${subListPath}</#if>" :key="item.sub_guid || index" :data-index="index" :sub-index="index" :class="{'transition':index==transitionIndex}" data-subname="data.${subListPath}" :row_readonly="item.sub_row_readonly">
                    ...
                    <#if layout.options.relation!='onetoone'>
                      <td align="center" <#if field.options.hideCtrl> v-show="false"</#if> class="trash printHide" v-if="permission.table.${layout.options.boSubEntity}.del">
