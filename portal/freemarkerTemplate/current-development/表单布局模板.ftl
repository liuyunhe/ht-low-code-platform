...
<#function transformLinkage hasLinkage>
<#local flag=false>
<#if hasLinkage??>
<#local flag=true>
</#if>
...
<#function getFormItem field type hasLinkage>
	...
	<#assign rtn>
		<#if field.ctrlType!='iframe' && field.ctrlType!='show-lc' && field.ctrlType!='contractPreview' && field.ctrlType!='table'>
			<#if field.noTitle>
				<@input field=field type=type hasLinkage=transformLinkage(hasLinkage)/>
			<#else>
				<#if  field.ctrlType!=''>
					<ht-form-item
						...
						<#if !field.options.hideLabel &&field.ctrlType!=''>
							<template slot="label">
								<#if field.options.tip?default("")?trim?length gt 1>
									<el-tooltip placement="top" <#if type!=1 && (transformLinkage(hasLinkage)==true)> v-permission-line-lable="'${field.parentNodeType}_${field.tableName}_${field.name}'" </#if> >
										<div slot="content" >${field.options.tip}</div>
										<i class="el-icon-question" />
									</el-tooltip>
								</#if>
								<span <#if type!=1 && (transformLinkage(hasLinkage)==true)> v-permission-line-lable="'${field.parentNodeType}_${field.tableName}_${field.name}'" </#if>  style="${util.getFieldStyle(field.options)}
		...
		<#elseif  field.ctrlType=='table'>
			<#if (field.rows?size > 0)>
				...
				<#assign layout =field.options.layOut!'auto'>
				<#assign topLine=field.options.topLine!{}>
				<#assign bottomLine=field.options.bottomLine!{}>
				<#assign openTop=topLine.open!false>
				<#assign openBottom=bottomLine.open!false>
				<el-main>
					<div class="table_layout_wrap">
                        <table data-colSum="${colSum}" width="100%" border="0" cellspacing="0" class="main-table" cellpadding="0" style="table-layout:${layout};border-right:${borderWidth}px solid ${borderColor};border-bottom:${borderWidth}px solid ${borderColor};">
											...
											<#list col.list as tdField>
												...
												<#if !tdField.options.hideCtrl &&tdField.ctrlType!=''>
													<#if !tdField.options.hideLabel &&tdField.ctrlType!=''>
														...
														<td v-if="${getPermission(tdField,type)}!='n'" class="main-title <#if tdField.options.validate?if_exists && tdField.options.validate?length gt 0> is-required</#if>"
														 ...
														 >
                                                            ...
                                                            <span <#if !tdField.options.noBindModel || util.isNoAttrField(tdField)> v-if="${getPermission(tdField,type)}!='n'" </#if> style="${util.getFieldStyle(tdField.options)}color:<#if formSetting && formSetting.fontColor>${formSetting.fontColor?replace('\"','')}<#else>${fontColor}</#if> !important;
                                                                 ${util.getDiyColorOrNull(tdField.options)}">${tdField.desc}<#if formSetting && formSetting.customColon!=true>：</#if></span>
														</td>
														...
														>
															<@input field=tdField type=type hasLinkage=transformLinkage(hasLinkage)/>
														</td>
													<#else>
						...
						<#if (openBottom=true)>
						<tr style="height:${bottomLine.height!'28px'}" >
						<td rowspan="1" colspan=${colSum*2}
						:style="{
						   wordWrap:'break-word',
						   wordBreak:'break-word',
						   textAlign:'${bottomLine.textAlign}',
						   fontFamily:'Microsoft YaHei',
						   fontSize:'${bottomLine.fontSize}',
						   borderLeft:${bottomLine.useBorder}?'${borderWidth}px solid ${borderColor}':'none',
						   borderTop:${bottomLine.useBorder}?'${borderWidth}px solid ${borderColor}':'none'
						}"
						>
						   <div style="font-size:inherit;padding:${bottomLine.padding!'10px'};font-color:${bottomLine.fontColor};">
						   	<span style="font-size:inherit;font-family:inherit;font-color:inherit;">${bottomLine.content}</span>
						   </div>
						</td>
						</tr>
						</#if>
