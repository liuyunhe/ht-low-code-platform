<template>
  <el-tabs type="card">
    ...
    <el-tab-pane label="查看数据">
      ...
      >
        <ht-table-column
          v-for="(xaxisField, index) in customChart.xaxisField"
          :key="index"
        ...
        <ht-table-column
          v-for="(displayfield, index) in customChart.displayfield"
          :key="index"
...
export default {
  ...
  mounted() {
    ...
    const that = this
    window.addEventListener('resize', that.resizeChart)
  },
  beforeDestroy() {
    const that = this
    window.removeEventListener('resize', that.resizeChart)
  },
  watch: {
    id: function (newV) {
      if (newV) {
  ...
  methods: {
    ...
    //ccwgq 2022-10-24 当窗口缩放时 echart图表对应缩放
    resizeChart() {
      const instance = this.instance
      if (instance?.resize ?? null) {
        instance.resize()
      }
    },
    buildChart(data) {
      ...
      let option = {
        ...
        tooltip: {},
        toolbox: {feature: {saveAsImage: {}}}, //dataZoom打开区域缩放
        legend: {},
        grid: grid,
        series: {},
      }
    ...
    buildLine(data, option) {
      ...
      //ccwgq 2022-10-24 管理端可配置柱状图宽度
      if (seriesData.length && (series?.barMaxWidth ?? 0) > 0) {
        seriesData.forEach((item) => {
          if (item['type'] == 'bar') {
            item['barMaxWidth'] = series.barMaxWidth || 200
          }
        })
      }
