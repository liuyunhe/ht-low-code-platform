        ...
        <el-col :span="8">
          ...
          <span style="margin-left: 20px">
            ...
            <el-button
              type="text"
              @click="deleteArress()"
              v-show="isShowAddress == 2"
              >删除抄送</el-button
            >
            <el-button
              type="text"
              @click="addCAddresses()"
              v-show="isShowCAdress == 1"
              >添加密送</el-button
            >
            <el-button
              type="text"
              @click="deltetCAddresses()"
              v-show="isShowCAdress == 2"
              >删除密送</el-button
            >
          </span>
        </el-col>
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  v-for="item in rows"
                  :key="item.id"
                  @click.native="selectAddress(item.mailAddress)"
                  >{{ item.mailAddress }}</el-dropdown-item
                >
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
...
export default {
    ...
    sendMail(action, data) {
      this.$refs[data].validate((valid) => {
        if (valid) {
          ...
          this.$store
            .dispatch('mail/saveMail', this.data)
            .then(() => {
              this.back()
            })
            .catch(() => {
  ...
  created() {
    ...
    this.$store
      .dispatch('mail/getMailSettingByPagination', pageBean)
      .then((data) => {
        for (let i of data) {
          if (i.isDefault == 1) {
