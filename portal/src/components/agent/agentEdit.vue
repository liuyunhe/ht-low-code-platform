  ...
  <el-container>
    ...
    <el-main>
      ...
      >
        ...
        <!--<el-form-item label="流程:" v-if="form.defList !=null">
                    <el-input ></el-input>
        </el-form-item>-->
...
export default {
  ...
  data() {
    return {
			form:{authNames:null,authNamesTemp:null},
      rules: {
        /* expre:[
      ...
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          this.$store
            ...
            .then(res => {
              this.$message({type:"success",message: "更新流程流程代理成功！"});
              this.$router.go(-1);
            });
        } else {
          return false;
  ...
  created() {
    this.$store.dispatch("agent/detail", this.id).then(data=>this.form={...data});
  }
};
</script>
