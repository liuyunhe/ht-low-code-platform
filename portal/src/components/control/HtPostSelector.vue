        ...
        <el-container>
          ...
          <el-main style="padding:0">
            <el-table
              ...
              @select="postSelectionChange"
              @select-all="postSelectionChange"
              class="org-table"
              size="medium"
            >
...
export default {
  ...
  computed: mapState({
    ...
    total: state => state.user.postTotal,
    dimensions: state => {
    ...
    inputVal: function() {
      if (!this.value) {
        ...
        if (!utils.getAndValidPath(this.bindSet, "name")) {
          throw "ht-post-selector的bind属性中必须配置name属性.";
        }
        if (this.value.constructor != String) {
          throw "ht-post-selector中有bind属性时，name必须是字符串.";
        }
        let subIndex = null;
        if (this.$el) {
        ...
        if (idPath) {
          ...
          if (!idVal || idVal.constructor != String) {
            throw "ht-post-selector中有bind属性中id的值必须是非空的字符串.";
          }
          idAry = idVal.split(",");
        }
        ...
        if (codePath) {
          ...
          if (!accountVal || accountVal.constructor != String) {
            throw "ht-post-selector中有bind属性中code的值必须是非空的字符串.";
          }
      ...
      setTimeout(() => {
        if (this.$store.state.user.currentUserDetail == null) {
          this.$store
            ...
            .then(postCharge => {
                this.postCharge = postCharge.post;
                this.selectOrgs = [this.postCharge];
                this.syncInputValue();
                this.calacInputSuffixHeight();
            });
        } else {
        this.postCharge = this.$store.state.user.postCharge.post;
        this.selectOrgs = [this.postCharge];
        this.syncInputValue();
        this.calacInputSuffixHeight();
        }
  ...
  created() {
    ...
    this.$store.dispatch("user/queryPost", pageBean);
  },
  ...
  methods: {
    ...
    handleNodeClick(data, node, vuecomponent) {
      ...
      this.$store.dispatch("user/queryPost", queryFilter);
    },
    search(str) {
      let querys = []; //查询条件
      let queryFilter = {};
      ...
      if (this.tableData == "" && this.orgId == "") {
        this.$store.dispatch("user/queryPost", pageBean).then(() => {
          this.syncCheckedStatus();
        });
      } else {
        ...
        this.$store.dispatch("user/queryPost", queryFilter).then(() => {
          this.syncCheckedStatus();
        });
      }
    },
    ...
    radioChangeHandler(index) {
      this.selectOrgs = [this.postData[index]];
    },
    postSelectionChange() {
      !this.updating && this.syncSelection2selectPosts();
    },
    postRowClick(row, column, event) {
      // 单选
      if (this.isSingle) {
        this.selectOrgs = [row];
      ...
      // 多选
        ...
        this.syncSelection2selectPosts();
      }
    },
    // 获取当前用作唯一键的参数
    getUniqueArg() {
