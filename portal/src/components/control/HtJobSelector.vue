        ...
        <el-container>
          ...
          <el-main style="padding:0">
            <el-table
              ...
              border
              height="490"
              @row-click="jobRowClick"
              @select="jobSelectionChange"
              @select-all="jobSelectionChange"
              class="org-table"
              size="medium"
            >
...
export default {
  ...
  computed: mapState({
    ...
    total: state => state.user.jobTotal,
    jobData: state => {
      return state.user.jobData;
    },
    inputVal: function() {
      if (!this.value) {
        return null;
        ...
        if (!utils.getAndValidPath(this.bindSet, "name")) {
          throw "ht-job-selector的bind属性中必须配置name属性.";
        }
        if (this.value.constructor != String) {
          throw "ht-job-selector中有bind属性时，name必须是字符串.";
        }
        let subIndex = null;
        if (this.$el) {
        ...
        if (idPath) {
          ...
          if (!idVal || idVal.constructor != String) {
            throw "ht-job-selector中有bind属性中id的值必须是非空的字符串.";
          }
          idAry = idVal.split(",");
        }
        ...
        if (codePath) {
          ...
          if (!accountVal || accountVal.constructor != String) {
            throw "ht-job-selector中有bind属性中code的值必须是非空的字符串.";
          }
  ...
  created() {
    ...
    this.$store.dispatch("user/queryJob", pageBean);
  },
  watch: {
  ...
  methods: {
    search(str) {
      ...
      if (this.tableData == "") {
        this.$store.dispatch("user/queryJob", pageBean).then(() => {
          this.syncCheckedStatus();
        });
      } else {
        querys = [
        ...
        this.$store.dispatch("user/queryJob", queryFilter).then(() => {
          this.syncCheckedStatus();
        });
      }
    },
    ...
    radioChangeHandler(index) {
      this.selectOrgs = [this.jobData[index]];
    },
    jobSelectionChange() {
      !this.updating && this.syncSelection2selectOrgs();
    },
    jobRowClick(row, column, event) {
      // 单选
      if (this.isSingle) {
        this.selectOrgs = [row];
    ...
    syncSelection2selectOrgs() {
      ...
      let curPageAry = this.selectOrgs.filter(m =>
        this.jobData.some(n => m[arg] === n[arg])
      );
