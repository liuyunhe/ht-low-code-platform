...
export default {
  ...
  methods: {
    ...
    //2022-06-21 ccwgq 修复千分位显示的方式
    ouThousanBit() {
      var _this = this
      var integer = new Number(_this.inputVal).toLocaleString() //直接转换,当没有小数点时补足
      var tail = ''
      var oriTail = integer.split('.')[1] || ''
      tail = oriTail.length == 0 ? '.00' : oriTail.length == 1 ? '0' : ''
      if (['r'].includes(this.permission)) {
        //只读
        _this.$nextTick(() => {
          var $tail = $(_this.$el).find('.field-tail')
          if ($tail.length == 1 && typeof _this.inputVal == 'number') {
            $tail.find('span').text(`${integer}${tail}`)
          }
        })
        ...
        _this.$nextTick(() => {
          ...
          if ($input.length == 1 && typeof _this.inputVal == 'number') {
            $input.val(`${integer}${tail}`)
          }
        })
      }
    },
    //2022-06-29 ccwgq 千分位显示改进，toLocalString兼容性存在问题
    ouThousanBit2() {
