<template>
  <div style="margin-bottom: 5px">
    <div ref="headHeight" :style="'height: '+headHigh+'px;'" v-if="defId==undefined"></div>
    <div class="divider" v-if="defId==undefined"></div>
    <div ref="doneContent" class="done_form" style="padding:20px;background: #fff">
...
export default {
  ...
  mounted(){
   this.$nextTick(()=>{
    ...
    const $wEl=$(parentEl).children('.btn_region');
    // console.log($wEl);
    if($wEl.length==1){
      const offsetTop=$wEl.prop('offsetTop');
      const offsetHeight=$wEl.prop('offsetHeight');
      const el=this.$refs.doneContent;
      const doneOffsetTop=$(el).prop('offsetTop');
      if((offsetTop+offsetHeight)>doneOffsetTop){
          this.headHigh=10
      }
      else if(doneOffsetTop-(offsetTop+offsetHeight)){
        this.headHigh=10;
      }
    }
   })
