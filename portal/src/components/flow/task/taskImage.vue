  ...
  <div class="bpm-btn-dialog">
    ...
    >
      ...
      <div style="overflow: auto;min-height:500px;">
        <div
          :style="{position:positionImg,width:widthImg+'px',height:heightImg+'px',background:'url('+backGroundImg+') no-repeat'}"
        >
          <el-popover
          ...
          >
            <div class="opinion-content" v-if="nodeOpinions[layout.nodeId]">
              ...
              <div v-if="nodeOpinions[layout.nodeId].hasOpinion">
                ...
                >
                      ...
                      <div class="owner-div">
                        ...
                        >
                          <el-button
                            ...
                            size="mini"
                            v-if="!qualfied.users"
                          >{{qualfied.name}}</el-button>
                          <el-tooltip
                            class="item"
                            effect="light"
                            v-if="qualfied.users"
                            :content="qualfied.users"
                            placement="top"
                          >
                            <el-button
                              type="primary"
                  ...
                  <tr>
                    <th>意见</th>
                    <td class="opinion" v-if="isHtml(op.opinion)" v-html="op.opinion"></td>
                    <td class="opinion" v-else>{{op.opinion}}</td>
                  </tr>
                </table>
              </div>
            ...
            <el-button
              v-if="layout.nodeType=='USERTASK' || layout.nodeType=='SIGNTASK'  || layout.nodeType=='CUSTOMSIGNTASK'"
              slot="reference"
              :style="{position:'absolute',border: 'none',background: 'none', left:layout.x+'px', top:layout.y+'px', width:layout.width+'px', height:layout.height+'px'}"
            ></el-button>
          </el-popover>
  ...
  methods: {
    handleOpen() {
      ...
      this.$store.dispatch("storeProcess/getBpmImage", data).then(res => {
        ...
        // lyzcw：2022-6-24，解决流程启动前查看流程图问题
        if( this.instId ){
          this.$store
...
.opinion-content /deep/ table > tr > th, .opinion-content /deep/ table > tr > td {
