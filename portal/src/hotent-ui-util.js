var utils = {
    ...
    // 获取孙表每一行所在的作用域元素及索引
    getSunScopeElAndIndex:(el)=>{
        const sunScopeEl = utils.getParentElementByAttribute(
            el,
            "data-sunname"
        );
        if (!sunScopeEl) {
            return { sunScopeEl: null, index: null };
        }
        let index = sunScopeEl.dataset["index"];
        if (index === undefined) {
            throw "要计算的孙表行未获取到index属性.";
        }
        index = Number.parseInt(index);
        return { sunScopeEl, index };
    },
