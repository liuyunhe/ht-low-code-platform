 ...
 * @version 05 单元格合并方案的调整
 * @version 06 签字意见字段的隐藏
*/

//模型名
var modelName = "fxsxsp";
//模型数据
var modelData = _this.data[modelName];
//主表名
var mainTableName = 'd_risk_issues_consult_approve';
...
setSerialNO("risk_cons_serial_no","serial_no");
//公文编号插入到右上角,传入td的colspan属性
insertSearialNo(20);
//合并单元格，行数下标从0开始，别忘记减一！！！
mergeCell(10, 3, '呈报部门意见')
//设置经办人宽度
...
function getMainDeptInfo() {
    ...
    return new Promise((resolve, reject) => {
        window.app.$http.post(`${window.context.form}/form/customDialog/v1/getListData?mapParam=&alias=yhzz`, param).then(resp => {
...
function addLastLine(colspan) {
    ...
    var text = "注：此呈报如需加盖公司印章，发起人应在“经办人意见”栏内明确提出加盖哪类印章" +
        "（如：东北证券公章、法人章及各类印章等），否则不予受理。"
    var $tr = $(`<tr style="height:28px;"><td  rowspan='1' colspan='${colspan}'` +
...
function insertSearialNo(colspan) {
    ...
    if (isStarted == '01' && !!mainModel['serial_no']) {
        //流程已经启动
        const $insert = $(`<tr><td colspan="${colspan}" style="text-align:right;"><span>公文编号：` +
            `【${mainModel['serial_no']}】</span></td></tr>`)
