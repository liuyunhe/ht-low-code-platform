/**
 * @author ccwgq
 * @description 外聘律师备案台账PC表单脚本
 * @version 1.0 参考履约台账的部分流程
 */

 const modelName = 'wplsbatz';
 const mainTableName = 'd_legal_lawer_ext_account';
 const subTableName = 'd_legal_lawer_ext_account_detail';
 //直接获取主表数据实体
 let mainModel = _this.data[modelName];
 //子表的数据实体
 let subTableData = _this.data[modelName]['sub_' + subTableName];
 //主表和子表的主权限
 ...
 //给公文编号赋值
setSerialNO("ex_lay_recod_no","serial_no");
//公文编号插入到右上角
insertSearialNo(20);
...
clearOpinion();
 ...
 function initPage() {
     ...
     if (subTableData.length > 0) {
         sortSubTableData(subTableData);
         _this.$nextTick(() => {
             subTableData.forEach((item, index) => {
  ...
  * @param {下标} index 
  */
 function addSubTableData(index) {
     //创建人姓名当前用户
     if (subTableData[index] && isEmpty(subTableData[index]['creater'])) {
         subTableData[index]['creater'] = currentUser.username;
         subTableData[index]['creater_id'] = currentUser.account;
         //记录人记录人id
         subTableData[index]['perf_users'] = currentUser.username;
         subTableData[index]['perf_users_id'] = currentUser.account;
         subTableData[index]['create_time'] = app.$Formulas.formatDate(new Date(), "YYYY-MM-DD HH:mm:ss")
     }
     cssMiddle(index);
 }
  ...
  */
 function addWatch() {
     const subTableDataKey = "data." + modelName + ".sub_" + subTableName + ".length";
     _this.$watch(subTableDataKey, function (nVal, oVal) {
         //发现脚本监听事件太多对性能造成了影响,做异步操作
         setTimeout(() => {
             if (nVal > oVal) {
                 //增加
                 addSubTableData(nVal - 1);
             } else if (nVal < oVal) {
                 //减少
             }
         }, 0)
  ...
  * @param sortKey
  */
 function sortSubTableData(needArray) {
  ...
  * 同步律师姓名,手填时同步
  */
 function syncLawerName(){
    const key=`data.${modelName}.tmp_1`;
    _this.$watch(key,function(val){
        if(val!==mainModel['ID_CARD_']){
            mainModel['lawyer_nm']=val
        }
    })

    const idCardKey=`data.${modelName}.ID_CARD_`;
    _this.$watch(idCardKey,function(val){
        mainModel['lawyer_id']=val;
    })
 }
