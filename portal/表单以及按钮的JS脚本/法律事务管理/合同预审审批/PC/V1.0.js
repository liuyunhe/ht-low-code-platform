/**合同预审审批,PC脚本
 * @version 02
 * @version 03 移除附件控制相关脚本,附件已经使用流程附件控件解决
 * @version 04 单布局方式调整为 fixed
 * @version 05 签字意见字段的隐藏
 */

//模型名
var modelName = "htyssp";
//模型数据
var modelData = _this.data[modelName];
//主表名
var mainTableName = "d_cont_early_approve";
...
//模型中的紧急程度和流程的同步
urgentStatusSync();
//给公文编号赋值,第一个字段是流水号的别名，第二个是给公文编号的字段名
setSerialNO("cont_preli_serial_no", "serial_no");
//公文编号插入到右上角,传入td的colspan属性
insertSearialNo(20);
//合并单元格，行数下标从0开始，别忘记减一！！！
// mergeCell(17, 3, '呈报部门意见')  //不需要合并
//设置经办人宽度
//setFieldCellWidth(190, 'perf_users')
//设置公文宽度
...
function mergeCell(startIndex, mergeLine, text, options = {}) {
  startIndex = mainModel["is_started"] == "00" ? startIndex : startIndex + 1;
  ...
  //生成DOM
  const insertTd = $(
    `<td rowspan="${mergeLine}" style="${style}"><span>${text}</span></td>`
  )[0];
...
//律师提前介入审批
function lawerEarlyRadio() {
  var fieldName = "is_lawer_invent";
  var $icon = $(`div[model-name="data.${modelName}.${fieldName}"]`)
    .parents("td")
    .siblings()
    .last()
    .find("i");
  _this.$watch(
    ...
    function (nVal, oVal) {
      if (nVal == "01") {
        //是
        // if(['b','w','r'].includes(permission["relate_early_bpm"])){
        //      permission["relate_early_bpm"]= ['b','w'].includes(permission['relate_bpm'])?'b':'r' //提前介入审批公文必填
        //      _this.$forceUpdate()
        // }
        // //icon隐藏
        $icon.hide();
        ...
        //否
        // permission["relate_early_bpm"]="n"  //公文隐藏
        // $(`div[model-name="data.${modelName}.${fieldName}"]`)
        //icon显示
        $icon.show();
      }
    },
