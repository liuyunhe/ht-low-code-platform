<template>
  ...
  >
    ...
    <vue-ueditor-wrap class="property_text" v-if="type=='property-text' &&!isEditor && permission_sub!='n'" v-model="inputVal" :config="config_text"></vue-ueditor-wrap>
    <vue-ueditor-wrap :ref="this.atter" v-model="inputVal" :config="config" v-if="isEditor && permission_sub!='n'"  ></vue-ueditor-wrap>
    <!--用来控制富文本校验-->
    <ht-input
      size="small"
      :validate="inputValidate"
...
export default {
  ...
  methods:{
    getObjectValue(text, t, v){
      ...
      // let val = text.substring(text.indexOf("{") +2, text.indexOf("}")).split(".");
      // if(val.length == 1){
      //   return text;
      // }
      // let check = new RegExp("[\u4e00-\u9fa5]");
...
div /deep/ .edui-default .edui-editor {
  z-index :1 !important
}
</style>