<template>
  <div >
      <el-row style="display:inline-block;vertical-align: top;" v-if="permission_sub!='n'">
        <el-tag
          ...
          :index="index">
          <el-tooltip class="item" effect="dark" :content="getImgFit(item)" placement="top">
            <el-image style="width: 80px; height: 26px" :ref="`signature_`+item.signature" :load="getShowSignature(item.signature)" :src="getShowSignature(item.signature)"></el-image>
          </el-tooltip>
          
        </el-tag>

        <el-button type="warning" v-if="permission_sub!=='r' && showSignatureBtn && false" @click="doSignature" icon="el-icon-s-check">签章</el-button>
      </el-row>
      <el-dialog
        title="获取签章"
        ...
        <van-field v-model="password" label="签章密码" type="password" placeholder="请输入密码" />
...
export default {
    ...
    signatureConfig: {
      handler: function (newVal, oldVal) {
        if(this.signatureConfig.field == this.fieldPath){
          ...
          Notify({type: "warning",message: "未获取到签章字段路径，请初始化表单模板后重新保存表单！"});
        }
      },
      deep: true,
      immediate: true,
  ...
  methods: {
    ...
    onConfirm(){
      if(!this.password){
        Notify({type: "danger",message: "请填写获取签章密码！"});
        return ;
      }
