    ...
    >
      <div>
        ...
        <el-header class="middle-header" v-if="querysShow">
          <el-input
            size="small"
            style="width: 98%; padding-top: 8px"
            clearable
        ...
        >
          <tbody v-for="(item, $index) in conditionfieldTree" :key="$index">
            <tr v-if="item.defaultType == '4'">
              <th width="130px" class="is-required">{{ item.comment }}:</th>
              <td>
                <ht-input
                  type="text"
                  v-if="item.type != 'date'"
...
export default {
  ...
  data() {
    return {
      ...
      nodeKey: 'ID_', //树形数据父Id
      customDialogShowList: false, //打开列表对话框
      customDialogShowTree: false, //打开组合对话框
      selectOrgProps: {
        label: 'name'
      },
      style: '', //对话框宽高
  ...
  computed: {
    ...
    total: function() {
      return this.$store.state.dialog.total
    },
    dialogData: function() {
      return this.$store.state.dialog.rows
    },
    inputVal: {
      set(val) {
        let this_ = this
        if (!this_.input_) this_.input_ = true
  ...
  methods: {
          ...
          userInputList.forEach(ele => {
            ...
            if (ele.defaultType == '1') {
              ...
              if (!this_.quickSearch) {
                this_.quickSearch = ele.comment
              } else {
                this_.quickSearch = this_.quickSearch + '、' + ele.comment
              }
            }
          })
        }
    ...
    /**@deprecated */
    // 同步选择结果-lyzcw: 内容没有URI编码，报错，废弃，2021-06-17 11:24
    syncInputValue1() {
      let subIndex = null
      if (this.$el) {
    ...
    dialogTreeConfirm() {
      // console.log('tree dialog confirm!')
      this.calacInputSuffixHeight()
      let subIndex = null
      if (this.$el) {
        subIndex = utils.getSomeAttributeFromParentElement(
