...
    }
    ...
    const me_ = this;
    var corpid = utils.getParameters("corpid");
    var redirect_uri = decodeURIComponent(utils.getParameters("redirect_uri"));
    dd.ready(function() {
      dd.runtime.permission.requestAuthCode({
        corpId: corpid,
        onSuccess: function(info) {
          me_.proxyLogin(info.code, redirect_uri, loading,params);
        },
        onFail: function(err) {
  ...
  methods: {
    proxyLogin(code, redirect_uri, loading,params) {
      let url = window.context.uc + "/sso/dingTalk?code=" + code;
    ...
    parseParams(params){
      ...
      //审批的消息
      if(paramsJson.templateType ==='bpmnapproval'|| paramsJson.templateType ==='bpmnback'){
