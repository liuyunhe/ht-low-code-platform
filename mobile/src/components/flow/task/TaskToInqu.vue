  ...
  >
    <div :style="this.style">
      <van-form v-form @submit="confirm">
        ...
        <div class="title-divider" v-if="status">
          <span>征询类型：</span>
        </div>
        <div class="opinion-text" v-if="status">
  ...
  methods: {
    ...
    confirm() {
      ...
      let data = {
        ...
        action: 'back',
        files: files,
        data: JSON.stringify(this.$parent.$parent.$refs.formContent.data),
      }
      //征询类型
      if (this.status) {
      ...
      const this_ = this
      this_.$store
        .dispatch('storeProcess/taskToInqu', data)
        .then((rep) => {
          loading.close()
          if (this_.token != null) {
            this_.inquDialog = false
