              ...
              <div class="opinion-content" v-if="nodeOpinions[layout.nodeId]">
                  ...
                  <div v-if="nodeOpinions[layout.nodeId].hasOpinion">
                    <table  v-for="op in nodeOpinions[layout.nodeId].data" :key="op.taskKey"  class="table table-bordered"> 
                        ...
                        <tr><th>开始时间</th>
                          <td style="font-size:11px;">{{op.createTime | dateformat}}</td>
                        </tr> 
                        <tr><th>结束时间</th>
                          <td style="font-size:11px;">{{op.completeTime | dateformat}}</td>
                        </tr> 
                        <tr><th>审批用时</th>
  ...
  methods: {
    handleOpen(defId,instId,subNodeId) {
        ...
        this.$store.dispatch("flow/getBpmImage",data).then(res=>{
            ...
            this.$store.dispatch("flow/getNodeopinions",{instId:res.rows.instanceId,nodeIds:nodeIds,defId:res.rows.defId}).then(resp=>{
              if (resp) {
